...
const ArtistList = () => {
    const { filters, tableState, setFilters, loading, refreshData } = useArtistManagementState();
    ...
    const onRowClick = (row: Row<Artist>) => {
        ...
        if (id) {
            openModal(
                <DialogArtistForm onClose={handleCloseArtistModal} data={row.original} />
            )
        }
    }
    ...
    return (
        <div>
            <PaginationTable<Artist>
                ...
                pagination={data?.meta ? data.meta : {
                    currentPage: 1,
                    hasNextPage: false,
                    hasPrevPage: false,
                    size: 50,
