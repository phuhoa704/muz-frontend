...
function NavigationMenu({ children, className, parentPath, id = 'sidebarMenu' }: Readonly<Omit<ItemProps, 'path'>>) {
    ...
    return (
        <ul className={cn('flex flex-col gap-2 transition-all duration-300 ease-in-out', `parent-${parentPath}`, className)} id={id}>
            {children}
        </ul>
    )
}

function NavigationMenuItem({ children, className, path }: Readonly<ItemProps>) {
    const control = useNavigationMenu();
    ...
    return (
        <li className={cn(className, isOpen && 'is-open', hasSelected && 'has-child-active' && isSelected && 'is-active')}>
            {children}
        </li>
    )
}
...
function NavigationMenuLink({ children, to, className, hasChild = false, path }: Readonly<NavigationMenuLinkProps>) {
    const control = useNavigationMenu();
...
export function SidebarNavMenu({
    ...
    parentPath = '',
    id = 'sidebarMenu',
}: Readonly<{ items: MenuItem[]; className?: string; parentPath?: string, id?: string }>) {
    ...
    return (
        <NavigationMenu className={cn(className)} parentPath={parentPath} id={id}>
            {items.map((item) => {
                ...
                return (
                    <ProtectedComponent action="read" subject={item?.subject} key={item.label}>
                        <NavigationMenuItem
                            key={item.label}
