...
const DialogFooter = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
    <div className={cn('flex flex-col-reverse sm:flex-row sm:justify-center sm:space-y-2', className)} {...props} />
)
DialogFooter.displayName = 'DialogFooter';

const DialogTitle = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Title>, React.ComponentPropsWithoutRef<typeof DialogPrimitive.Title> & { showCloseButton?: boolean, onClose?: () => void }>(({ className, onClose, showCloseButton = false, ...props }, ref) => (
    <div className='flex justify-between items-center'>
        ...
        <DialogPrimitive.Close
            className={cn('rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted', { hidden: !showCloseButton })}
            onClick={() => {
                onClose?.();
            }}
        >
